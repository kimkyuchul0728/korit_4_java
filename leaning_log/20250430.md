...
- '모든 클래스는 Object 클래스를 상속 받는다.' 그래서 interface나 다른 상속 관련 수업에서
    ...
    1. toString() - 기본적으로 모든 클래스는 Object 클래스를 상속 받아, Object의
        ...
        사용)를 반환함. -> 이를 override라고 재정의하여 필드를 표현하는 등 다양한 방식
        으로 사용하는 편.
        - 왜 사용하는가? - 사람들이 읽기 편한 방식으로 재정의하고, '객체명만으로'
            불러오는 것이 가능함. 즉 객체명.toString()형태로 쓰지 않고, 객체명만으로
            메서드를 호출하는 것이 가능(바로 코드로 보여드릴 예정입니다.)
    
    2. equals() - 두 객체가 '논리적으로 같은지'를 비교하는 메서드. 기본적으로 Object
        equals() 메서드는 구 객체의 참조 주소를 비교합니다.
    ...
    3. hashCode() - 객체를 정수값(해시값)으로 변환(16진수가 아니라). 해시 값은 주로
        ...
        동일한 객체는 공일한 해시 코드를 가져야하기 때문인데, 이게 문제가 좀 있는게
...
## 제대로 설치됐는지 검증
UserEmtityLombok에서 public class 어쩌고 위에 @Setter 등 했을 때 lombok으로 자동완성되는지 확인
...
@AllArgsConstructor
public class KoreaItStudent4 extends KorItStudent {
    // static 변수 선언 및 초기화
...
public class KorItStudent4Main {
    public static void main(String[] args) {
        ...
        KoreaItStudent4 student2 = new KoreaItStudent4("김이", 21);
        KoreaItStudent4 student3 = new KoreaItStudent4("김삼", 22);
        KoreaItStudent4 student4 = new KoreaItStudent4("김사", 23);
    }
}
```
...
## 정의
소프트웨어 '디자인 패턴' 중 하나로, 어떨 클래스가 하나의 인스턴스만 갖도록 보장하고, 그 인스턴스에
...
가독성과 유연성을 제동하는 것이 목표

## 특징
1. 객체를 생성하는 여러 개의 필드를 '명시적'으로 관리할 수 있음
2. 불칠요한 생성자 오버로딩을 줄여 가독성을 높임.
...
2. 생성자 오버로딩 문제
    - 생성자에 필요한 조합이 다른 경우 수 많은 생성자를 하나하나 정의해야함(이건 롬복으로도 한계가 있습니다.)
    - 유지 보수에 악영향 / 필드 하나 추가하면 생성자를 하나하나 뜯어고쳐야함 -> 제가 지금 저 위에 Student 클래스에
        douvle score 추가하고 생성자 정의하라고 하는 순간 코드 라인의 수는 엄청 늘어나게 될겁니다.
3. 가독성과 유지보수 문제
