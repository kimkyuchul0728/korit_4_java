...
/*
    List는 순서가 있고 중복을 허용하는 반면 Set는 순서는 없고, 중복은 허용하지 않습니다.
    (수학에서의 집합 개념을  Collection으로 구현한 것이 Set에 해당합니다.)
    그래서 특성의 차이로 인해 List -> set / Set -> List로의 형변환이 매우 중요합니다.
    List를 통해 전체 설문을 받고, Set을 통해 중복을 제거하여 후보군을 남기는 등의
    응용이 가능하기 때문에, 혹은 Set을 List로 바꿔 .ger(인덱스 넘버)를 통해 조회를하기 위해
    서로 간의 형변환이 매우 중요합니다.
 */
public class StrSet {
...
//        }                                                 // 얘는 Set 상황에서는 작성이 좀 애매합니다.
        ...
        // 새로운 Set을 선언해서 다시 strList에 있는 부분을 다 더해버리면 중복이 제거되엤네요.
